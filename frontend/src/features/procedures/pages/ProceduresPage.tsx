...
export const ProceduresPage = () => {
    ...
    const handleLinkProcedure = async (procedureId: string) => {
        ...
        try {
            await incidentService.linkProcedure(linkToIncidentId, procedureId);
            toast.success('Procedure linked successfully!');
            navigate(`${APP_PATHS.incidents}/${linkToIncidentId}`);
        } catch (error) {
            console.error('Failed to link procedure', error);
            toast.error('Failed to link procedure');
        }
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
                ...
                {!linkToIncidentId && (
                    <CanAccess permission="PROCEDURE_CREATE">
                        <Link
                            to={APP_PATHS.procedureNew}
                ...
                {procedures.slice((page - 1) * ITEMS_PER_PAGE, page * ITEMS_PER_PAGE).map((procedure) => (
                    <div key={procedure.id} className="block group">
                        {linkToIncidentId ? (
                            ...
                            <Link to={`${APP_PATHS.procedures}/${procedure.id}`} className="block">
