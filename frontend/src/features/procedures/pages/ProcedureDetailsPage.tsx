...
export const ProcedureDetailsPage = () => {
    ...
    const handleDelete = async () => {
        if (!await confirm('Delete this procedure?', 'This action cannot be undone.', 'danger')) return;
        try {
    ...
    useEffect(() => {
        const fetchProcedure = async () => {
            try {
                const data = await procedureService.getById(id!);
                setProcedure(data);
            } catch (error) {
    ...
    return (
        <div className="space-y-6">
            <Link to={APP_PATHS.procedures} className="inline-flex items-center text-sm text-slate-500 hover:text-slate-700">
                ...
                <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="flex flex-col items-end gap-2">
                        ...
                        <div className="flex gap-2 mt-2">
                            <CanAccess permission="PROCEDURE_EDIT">
                                <Link
                                    to={`${APP_PATHS.procedures}/${id}/edit`}
                            ...
                            <CanAccess permission="PROCEDURE_DELETE">
                                <button
                ...
                <div className="space-y-4">
                    {procedure.incidents && procedure.incidents.map((incident: Incident) => (
                        <div key={incident.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-md border border-slate-200">
                            <div>
                                <Link to={`${APP_PATHS.incidents}/${incident.id}`} className="font-medium text-primary hover:text-accent">
                                    {incident.title}
                                </Link>
