...
import { systemService } from '../../admin/api/admin.service';
...
export const CreateProcedurePage = () => {
        ...
        if (isEditMode) {
            const fetchProcedure = async () => {
                try {
                    ...
                    setValue('systemId', procedure.system?.id || '');

                    if (procedure.job?.id) setTimeout(() => setValue('jobId', procedure.job!.id), 500);
    ...
    useEffect(() => {
        if (!isEditMode && fromIncidentId) {
            const fetchIncident = async () => {
                try {
                    const incident = await incidentService.getById(fromIncidentId);
                    ...
                    if (incident.system?.id) setValue('systemId', incident.system.id);
                    if (incident.job?.id) setTargetJobId(incident.job.id);
    ...
    const onSubmit = async (data: CreateProcedureFormValues) => {
        try {
            ...
            if (isEditMode) {
                await procedureService.update(id!, payload);
            } else {
                const newProcedure = await procedureService.create(payload);

                if (fromIncidentId && newProcedure.id) {
                    try {
                        await incidentService.linkProcedure(fromIncidentId, newProcedure.id);
                    } catch (linkError) {
                        console.error('Failed to link procedure to incident', linkError);
                        toast.error('Procedure created, but failed to link to incident.');
                    }
                }
            }
        ...
        <div className="max-w-4xl mx-auto space-y-6">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-white p-6 rounded-lg shadow-sm border border-slate-200">
                ...
                <div className="flex justify-end gap-3 pt-4 border-t">
                    <button
                        type="button"
                        onClick={() => navigate(APP_PATHS.procedures)}
