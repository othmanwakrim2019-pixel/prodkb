...
export const NotificationBell = () => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {isOpen && (
                <div className="absolute right-0 top-full z-50 mt-2 w-96 overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-2xl">
                        ...
                        {notifications.length === 0 ? (
                            ...
                            notifications.map((notification) => (
                                <button
                                    ...
                                    className={`w-full px-4 py-3 text-left transition-colors hover:bg-slate-50 ${!notification.isRead ? 'bg-blue-50/40' : ''}`}
                                    type="button"
                                >
                                    <div className="min-w-0">
                                        <div className="flex items-center gap-2">
                                            <p className={`truncate text-sm ${!notification.isRead ? 'font-semibold text-slate-900' : 'font-medium text-slate-600'}`}>
                                                {notification.title}
                                            </p>
                                        ...
                                        <p className="mt-1 text-[10px] text-slate-400">{timeAgo(notification.createdAt)}</p>
                                    </div>
                                </button>
                            ))
                        )}
