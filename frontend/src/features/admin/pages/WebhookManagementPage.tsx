...
export const WebhookManagementPage = () => {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.error(axiosErr.response?.data?.message || 'Failed to save webhook');
        }
    };
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            ...
            await fetchWebhooks();
        } catch (err) { toast.error('Failed to delete webhook'); }
    };

    const handleToggle = async (w: Webhook) => {
        try {
            await webhookService.update(w.id, { isActive: !w.isActive });
            await fetchWebhooks();
        } catch (err) { toast.error('Failed to toggle webhook'); }
    };
