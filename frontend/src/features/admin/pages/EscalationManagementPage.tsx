...
import { Plus, Pencil, Trash2, AlertTriangle, Zap, Search } from 'lucide-react';
import { Pagination } from '../../../components/ui/Pagination';
import { escalationRuleService, systemService, teamService, type AdminSelectOption, type EscalationRule } from '../api/admin.service';
...
export const EscalationManagementPage = () => {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            if (editing) {
                await escalationRuleService.update(editing.id, payload);
            } else {
                await escalationRuleService.create(payload);
            }
            setShowForm(false);
            await fetchAll();
        } catch (err: unknown) {
    ...
    const handleDelete = async (id: string) => {
        if (!await confirmAction('Delete this escalation rule?', 'This action cannot be undone.', 'danger')) return;
        try {
            await escalationRuleService.delete(id);
            await fetchAll();
    ...
    const handleToggle = async (r: EscalationRule) => {
        try {
            await escalationRuleService.update(r.id, { isActive: !r.isActive });
            await fetchAll();
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-3">
                <h2 className="text-base font-semibold text-slate-900 dark:text-white self-start flex items-center gap-2">
                    <AlertTriangle className="h-4 w-4 text-slate-500" /> Escalation Rules
                </h2>
            ...
            {showForm && (
                <form onSubmit={handleSave} className="ent-card p-4 space-y-4">
                    <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">{editing ? 'Edit Rule' : 'New Escalation Rule'}</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required minLength={2}
                                className="ent-input" placeholder="e.g. Critical DB escalation" />
                        </div>
                        <div>
                            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Level (1–10) *</label>
                            <input type="number" value={form.level} onChange={e => setForm({ ...form, level: parseInt(e.target.value) || 1 })}
                                min={1} max={10} className="ent-input" />
                        </div>
                        <div>
                        ...
                        <div>
                            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Delay (minutes) *</label>
                            <input type="number" value={form.delayMinutes} onChange={e => setForm({ ...form, delayMinutes: parseInt(e.target.value) || 1 })}
                                min={1} className="ent-input" />
                        </div>
                    </div>
                    ...
                    <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                        <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
                            <tr>
                                ...
                                <th className="px-4 py-2 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Level</th>
                                <th className="px-4 py-2 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">System</th>
                                ...
                                <th className="px-4 py-2 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Escalate to</th>
                                <th className="px-4 py-2 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Delay</th>
                        ...
                        <tbody className="bg-white dark:bg-slate-900 divide-y divide-slate-200 dark:divide-slate-800">
                            {filtered.slice((page - 1) * limit, page * limit).map(r => (
                                <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-none">
                                    <td className="px-4 py-2 text-sm font-medium whitespace-nowrap text-slate-900 dark:text-slate-200">{r.name}</td>
                                    <td className="px-4 py-2 whitespace-nowrap">
                                        <span className="inline-flex items-center rounded px-1.5 py-0.5 text-[11px] font-bold uppercase tracking-wider bg-orange-600 text-white dark:bg-orange-900/40 dark:text-orange-400">
                                            <Zap className="w-3 h-3 mr-1" /> L{r.level}
                                        </span>
                                    </td>
                                    <td className="px-4 py-2 text-sm text-slate-600 dark:text-slate-300 whitespace-nowrap">{r.system?.name || <span className="text-slate-400 italic">All</span>}</td>
                                    <td className="px-4 py-2 text-sm text-slate-600 dark:text-slate-300 whitespace-nowrap">{r.severity || <span className="text-slate-400 italic">All</span>}</td>
                                    <td className="px-4 py-2 text-sm text-slate-700 dark:text-slate-300 whitespace-nowrap">{r.team?.name || '—'}</td>
                                    <td className="px-4 py-2 text-sm text-slate-600 dark:text-slate-300 whitespace-nowrap">{r.delayMinutes}m</td>
                                    <td className="px-4 py-2 whitespace-nowrap">
                                        <button onClick={() => handleToggle(r)} className={`inline-flex items-center rounded px-1.5 py-0.5 text-[11px] font-bold uppercase tracking-wider outline-none ${r.isActive ? 'bg-emerald-600 text-white dark:bg-emerald-900/40 dark:text-emerald-400' : 'bg-slate-200 text-slate-800 dark:bg-slate-700 dark:text-slate-300'}`}>
                                            {r.isActive ? 'Active' : 'Inactive'}
                                        </button>
                                    </td>
                                    <td className="px-4 py-2 text-right whitespace-nowrap">
                                        <div className="flex gap-1 justify-end">
                                            <button onClick={() => openEdit(r)} className="p-1 text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 rounded transition-colors"><Pencil className="w-4 h-4" /></button>
                                            <button onClick={() => handleDelete(r.id)} className="p-1 text-slate-400 hover:text-red-600 dark:hover:text-red-400 rounded transition-colors"><Trash2 className="w-4 h-4" /></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
