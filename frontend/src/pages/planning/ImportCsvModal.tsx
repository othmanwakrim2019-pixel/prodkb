...
export const ImportCsvModal = ({ isOpen, onClose, onImported }: ImportCsvModalProps) => {
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            const res = await axios.post('/api/v1/planning/import', formData, {
                                            ...
                                            {parsedRows.map((row, i) => (
                                                <tr key={i} className="hover:bg-slate-50/50">
                                                    ...
                                                    <td className="px-3 py-2">
                                                        <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${row.task_type?.toUpperCase() === 'BATCH' ? 'bg-blue-100 text-blue-700' : 'bg-violet-100 text-violet-700'}`}>
                                                            {row.task_type?.toUpperCase() === 'BATCH' ? '⚙️ BATCH' : '👤 MANUAL'}
                                                        </span>
                                                    </td>
                                                    <td className="px-3 py-2 text-slate-600">{row.date}</td>
                                                    <td className="px-3 py-2 text-slate-600">{row.time || '08:00'}</td>
                                                    <td className="px-3 py-2 text-slate-700 max-w-[200px] truncate" title={row.task_name}>
                                                        {row.task_name || `${row.system_name} / ${row.job_code}`}
                                                    </td>
                                                    <td className="px-3 py-2 text-slate-500 truncate max-w-[100px]">{row.support_contact || '—'}</td>
                                                    <td className="px-3 py-2 font-mono text-slate-400">{row.depends_on || '—'}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
